...
"""
    build_rtree(geom_arr, data_arr=[nothing])
...
relevant values get precomputed and cashed in the prepared geometry, rather than precomputed on every test. `data` holds the entry in `data_arr` at the index of
`orig` in `geom_arr`.
...
cashed in the prepared geometry, rather than precomputed on every test.
...
The `data` entry is a reference to the row of the original dataframe, providing access to all relevant data.
"""
